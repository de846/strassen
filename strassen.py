...
def strassen_solution(a,b):
    ...
    """Checking condition for whether the rows and columns satisfy the condition to be split into 4 equal quarters. If condition is true, split the matrix and create the 10 sub matrices"""
    if row_len_a == col_len_a and row_len_b == col_len_b and col_len_a == row_len_b and row_len_a % 2 == 0:
        ...
        """Implement the recursive condition. The base case is when the original matrix is of size 2. When it reaches the base case, multiplication is done using the regular method to ensure that the product is a 2D array. This ensures easy merging with the merge function"""
        if row_len_a > 2:
